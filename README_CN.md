...
- **多维数据整合**: 自动获取 FRED (国债收益率、CPI 通胀、失业率) 和 Yahoo Finance (S&P 500、VIX、汇率) 的关键宏观指标。
- **经济周期定位**: 基于利差、通胀和就业数据，量化评估当前所处的经济周期阶段 (如复苏、过热、滞胀等)。
- **风险环境评估**: 综合 VIX 波动率、美元指数 (DXY) 和套息交易 (USD/JPY) 信号，判定市场风险偏好 (Risk On/Off)。
- **资产配置建议**: 结合股权风险溢价 (ERP) 和估值水平，生成资产配置 (股/债) 和地域偏好 (美国/新兴市场) 建议。
- **双语报告**: 自动生成包含原始数据表格的中英双语 Markdown 报告。
